...
export class Lokichart {
    ...
    drawKeisen(ctx: CanvasRenderingContext2D | null, can: HTMLCanvasElement) {
        ...
        ctx.lineTo(can.width - this.keisenMargine + 3, can.height - this.keisenMargine)
        ctx.stroke();
    }

    drawMemori(ctx: CanvasRenderingContext2D | null, x: number, y: number, width: number) {
        if (ctx == null) {
            return;
        }
        ctx.fillStyle = color.border
    ...
    drawBar(data: graphData, ctx: CanvasRenderingContext2D | null, can: HTMLCanvasElement) {
        if (ctx == null) {
            return;
        }
        ...
        this.rightMargin = this.width - (this.keisenMargine + (this.barWidth + this.barhMargine) * count - this.barWidth)
        ...
        ctx.textAlign = "end";
        ctx.fillText(maxPrice.toString(), this.keisenMargine - 2, this.keisenMargine + 10)
        ctx.fillText(" 0", this.keisenMargine - 2, can.height - this.keisenMargine)

        const memoriCount = Math.floor(maxPrice / Math.ceil(maxPrice / 10))
        for (let i = 0; i < memoriCount; i++) {
            this.drawMemori(ctx, this.keisenMargine, this.keisenMargine + Math.ceil((this.graphHeight / memoriCount)) * i, this.graphwidth)
        }
        ...
        for (let b = 0; b < count; b++) {
            ...
            ctx.textAlign = "left";
            // x軸のlabel表示
            ctx.fillText(data.label[b], this.keisenMargine + this.barhMargine * 1.5 + (this.barWidth + this.barhMargine) * b, barLabel)

            ctx.fillStyle = "#0063B1";
            ctx.fillRect(this.keisenMargine + this.barhMargine + (this.barWidth + this.barhMargine) * b, can.height - this.keisenMargine - 1, this.barWidth, -this.graphHeight * data.value[b] / maxPrice)
        }
    }
    ...
    drow(e: MouseEvent, ctx: CanvasRenderingContext2D | null) {
        ...
        if (e.offsetX <= this.keisenMargine + this.barhMargine) {
            ...
            this.n.x = Math.floor((e.offsetX - this.keisenMargine) / (this.barWidth + this.barhMargine)) * (this.barWidth + this.barhMargine) + (this.keisenMargine + this.barhMargine)
        }
        ...
        // // カーソルがある場所に縦の線を引く
        ctx.fillRect(this.n.x, this.keisenMargine, this.barWidth, this.graphHeight);

        let selectedBar = Math.floor((e.offsetX - this.keisenMargine) / (this.barWidth + this.barhMargine));
        if (selectedBar < 0) {
            selectedBar = 0
        ...
        ctx.fillText(`${this.convertLabel(data.label[selectedBar])}`, this.n.x, this.keisenMargine - 24);
        ctx.fillText(`${data.value[selectedBar].toLocaleString()}${data.unit}`, this.n.x, this.keisenMargine - 10);
    }
